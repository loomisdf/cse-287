...
void drawVerticalLine(glm::vec3 p1, glm::vec3 p2, const color & rgba)
{
	if (p2.y < p1.y) {
	...
	for (float y = p1.y; y < p2.y; y++) {
		...
		frameBuffer.setPixel(p1.x, y, rgba);
	}

} // end drawVerticalLine

void drawHorizontalLine(glm::vec3 p1, glm::vec3 p2, const color & rgba)
{
	if (p2.x < p1.x) {
	...
	for (float x = p1.x; x < p2.x; x++) {
		...
		frameBuffer.setPixel(x, p2.y, rgba);
	}

} // end drawVerticalLine


void drawBresenhamLine(glm::vec3 p1, glm::vec3 p2, const color & rgba) {

	int x0 = (int)(p1.x + 0.5f);
	int y0 = (int)(p1.y + 0.5f);
	int x1 = (int)(p2.x + 0.5f);
	int y1 = (int)(p2.y + 0.5f);
	...
	frameBuffer.setPixel(x0, y0, rgba);
...
void drawWireFrameTriangle(const vector<VertexData> & vertices, const color & rgba)
{
	if (vertices.size() > 2) {
		...
		vector<glm::vec3> l0;
		l0.push_back(vertices[0].position.xxy);
		l0.push_back(vertices[1].position.xxy);
		vector<glm::vec3> l1;
		l1.push_back(vertices[1].position.xxy);
		l1.push_back(vertices[2].position.xxy);
		vector<glm::vec3> l2;
		l2.push_back(vertices[2].position.xxy);
		l2.push_back(vertices[0].position.xxy);

		frameBuffer.setV0V1V2(vertices[0], vertices[1], vertices[2]);
...
void drawFilledPolygon(vector<glm::vec4> vertices, const color & rgba) {
...
bool allSameY(vector<VertexData> vertices)
{
	if (vertices[0].position.y == vertices[1].position.y  && vertices[1].position.y == vertices[2].position.y) {
		return true;
	}
	else {
		return false;
...
void drawFilledTriangle(const vector<VertexData> & vertices, const color & rgba)
{
	...
	for (VertexData va : vertices) {
		v.push_back(va.position);
	}
